...
function modalMore2(id_konten, foto_profil, nama_group, username, tempat, media, caption, tanggal, slug) {
    swal.fire({
        html: '<ul class="list-group list-group-flush">' +
            ...
            '<li class="list-group-item"><a data-id="' + id_konten + '" image="' + foto_profil + '" data-group="' + nama_group + '" data-username="' + username + '" data-tempat="' + tempat + '" data-media="' + media + '" data-caption="' + caption.split('&').join('&amp;').split('<').join('&lt;').split('"').join('&quot;').split("'").join('&#39;') + '" data-tgl="' + tanggal + '" data-slug="' + slug + '" style="cursor: pointer; font-weight: 600; color: blue;" id="editKonten"> Edit </a></li>' +
            // '<li class="list-group-item"><a onclick="modalReportKonten(' + id_konten + ')" id="reportKonten" href="#" style="font-weight: 600; color: red;"> Report </a></li>' +
            '<li class="list-group-item"><a id="salinLink" content="' + id_konten + '" url="' + window.location.origin + '/sosial-media/group/p/' + slug + '" style="cursor: pointer;"> Salin Link </a></li>' +
...
function balas_komen(balas, id_balas_komen, username, id_konten) {
    let html = '';
    html += '<input type="hidden" name="id_balas_komen" class="id_balas_komen" value="' + id_balas_komen + '"><input type="hidden" name="username" value="' + username + '" class="username_balas"></input><textarea placeholder="Post your comment" name="isi_komentar" style="width: 100%;" class="txt_comment_' + id_konten + '">' + balas + ' </textarea>';
            ...
            if (data.length !== 0) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i].id_balas_komen == 0) {
                        html += '<li id="comment_' + data[i].id_cmt + '" class="list-rep_cmt_' + data[i].id_cmt + '" style="width:100%";>' +
                        ...
                        html += '<ul id="comment_' + data[i].id_cmt + '" style="margin-left: 80px;">' +
                            '<li>' +
                            '<div class="comet-avatar" style="width:5%;">' +
                            ...
                            '<div class = "coment-head">' +
                            '<h5 style = "text-transform: none;" > <a href = "/sosial-media/profil/' + data[i].username + '" title = "">' + data[i].username + '</a></h5>' +
                            '<span style="text-overflow: ellipsis;white-space: nowrap;">' + data[i].tanggal_komen + '</span>' +
                            '<a onclick = "modalHapusKomentar(' + data[i].id_cmt + ')" style = "cursor: pointer" ><i class = "ti-trash hide" style = "color: red;"></i></a>' +
                            // '<a onclick="modalReportKomentar(' + data[i].id_cmt + ')" style="cursor: pointer" title="Report Komentar"><i class="ti-info-alt hide" style="color: red;"></i></a>' +
                    ...
                    $('.thumb-xs' + data[i].id_konten).html('<textarea placeholder="Post your comment" name="isi_komentar" style="width: 100%;" class="txt_comment_' + data[i].id_konten + '"></textarea>');
                }
            }
        }
    });
...
$(document).ready(function () {
    document.getElementById('pro-imagee').addEventListener('change', readImage3, false);
